...
#
作用：
- ICP：回环的时候用，检测两个帧足够相似的特征，通过配对当前帧3D和之前帧3D求Rt
- PnP：重定位的时候用，检测两个帧足够相似的特征，通过配对当前帧的2D和之前帧的3D求Rt
- Triangulation：初始化的时候用，主要用于已知配对的2D和Rt求3D点
- E矩阵和H矩阵：初始化的时候用，通过两帧配对的2D恢复出Rt（中间会用三角化来check）

细节：
- ORB中这些运算统一运用RANSAC，界定内点个数，随机选一组点进行计算，然后check 选出内点最多的解，然后可以Refine一下送出去。
- check的方式最终都变成了平面内像素的距离误差，有匹配点的算重投影误差，E矩阵直接就是匹配点和投影过去的极线的距离
- 大部分关键运算通过DLT都转换Ax=0的形式，通过SVD分解的形式求最小奇异值对应的特征向量。
- 很多计算首先需要对数据进行归一化处理，一方面是为了推导（ICP直接用相对中线点的坐标、EPnP控制点的概念，2D-2D 放到归一化平面 约束尺度问题）一方面放到归一化平面算一定程度上减小误差
- 还有一些容易忽视的约束问题：**正交约束**旋转矩阵RTR=I、**尺度约束**Ax=0保证||x||=1、**奇异值约束**F和E矩阵的秩为2奇异值矩阵的第三行强置为0
#
